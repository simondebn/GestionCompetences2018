<header>

    <nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-cefim scrolling-navbar">
        <a class="navbar-brand" href="#"><strong>CEFIM</strong></a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse ml-lg-5" id="navbarSupportedContent">
            <!-- Search form -->
            <form class="form-inline md-form mr-auto mb-0">
                <input class="form-control w-75 mr-3" type="text" placeholder="Rechercher" aria-label="Search">
                <i class="fa fa-search text-white" aria-hidden="true"></i>
            </form>
            <ul class="navbar-nav">
                <li class="nav-item active">
                    <a class="nav-link waves-effect waves-light" href="#">Modifier son profil</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link waves-effect waves-light" href="#">Déconnexion</a>
                </li>
            </ul>
        </div>
    </nav>

</header>
<div class="container">